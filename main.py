...
class Fall_detector:
    def __init__(self, index, output_file = ''):
        ...
        self.par.add_argument('--detection_input_size', type=int, default=384,
        ...
        self.par.add_argument('--save_out', type=str, default=output_file,
                            help='Save display to video file.')
        self.par.add_argument('--device', type=str, default='cuda',
                            help='Device to run model on cpu or cuda.')
        self.args = par.parse_args(args=[])
    ...
    def load_models(self):
        # DETECTION MODEL.
        inp_dets = self.args.detection_input_size
        detect_model = TinyYOLOv3_onecls(inp_dets, device=device)

        # POSE MODEL.
        inp_pose = self.args.pose_input_size.split('x')
        inp_pose = (int(inp_pose[0]), int(inp_pose[1]))
        pose_model = SPPE_FastPose(self.args.pose_backbone, inp_pose[0], inp_pose[1], device=device)
    ...
    def detect(self, vid_input = 0):
        self.args.camera = vid_input
        cam_source = vid_input
        ...
        if self.args.save_out != '':
            ...
            writer = cv2.VideoWriter(self.args.save_out, codec, 30, (inp_dets * 2, inp_dets * 2))
